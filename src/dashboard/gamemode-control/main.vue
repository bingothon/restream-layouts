<template>
    <v-app>
        <v-radio-group v-model="gameMode" @change="updateGame">
            <v-radio v-for="game in ALL_GAME_MODES" :key="game" :label="game.name" :value="game.id"> </v-radio>
        </v-radio-group>
    </v-app>
</template>
<script lang="ts">
    ...
    export default class GameModeControl extends Vue {
        ...
        ALL_GAME_MODES = [
            { name: 'Super Mario Sunshine', id: 'sms' },
            { name: 'Breath of the Wild', id: 'botw' },
            { name: 'Sonic Adventure 2: Battle', id: 'sa2b' },
